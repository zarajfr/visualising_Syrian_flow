...
colors = []
for i in range(18):
    colors.append("#80013f")
palette = sns.color_palette(colors)
g = sns.relplot(
    data=df_data,
    x="month", y="flow_count", col="destination", hue="destination",
    kind="line", palette=palette, linewidth=2, zorder=5,
    col_wrap=3, height=1.7, aspect=2.3, legend=False, facet_kws={"sharey":False},
)
...
plt.legend(title = "Dflows", loc = 'upper left', labels = ['data','simulation'], bbox_to_anchor = (0.9,1.15))
g.savefig('social-5-exs.png')
